...
| **FR-14** | 헤드라인 종목별 매칭 정확도 (운영 v3) | yfinance.Ticker.news가 같은 일반 기사를 여러 ticker에 매칭하는 문제 해결. 2-pass: (1) title에 ticker symbol 또는 영문 회사명(`COMPANY_NAMES_EN`) 포함 헤드라인 우선, (2) 없으면 일반 헤드라인 fallback. `fetch_news_all` 종단에 dedupe — 같은 헤드라인이 여러 ticker에 결과되면 첫 등장만 유지, 나머지는 `top_headline=None` (어닝스/인사이더는 보존, `dataclasses.replace`) |
| **FR-15** | 메시지 헤더 시장 동향 한 줄 (운영 v3) | 헤더 직후 `📊 상승 N / 하락 M / VIX X.X (안정/경계/공포)` 라인 추가. 한 눈에 시장 톤 파악 |
| **FR-16** | 단타 후보 정렬 (운영 v3) | 신호 수 내림차순, 동률은 변동률 절댓값 내림차순. 강한 신호 먼저 노출 |
| **FR-17** | 초보자 가이드 (운영 v3) | (a) 메시지 끝 푸터 한 줄: `💡 신호: 🔥거래량 🎯갭 🆙신고가 📊시간외 ⚡VIX급등 ★사상최고`. (b) 단타 후보 사유 한글 구체화: 단순 "거래량 + 갭" 대신 "거래량 8.0× + 갭 +12.0%" 처럼 배수/% 표시 |
| **FR-19** | 종목 라인 압축 (운영 v3) | 모바일 슬랙 가독성. `• 종목 TKR: 142.30  ▲ +2.10 (+1.50%) 🟢 🔥` → `• 종목 TKR: 142.30 ▲ +1.50% 🟢 🔥`. 절대 변동 (`+2.10`) 생략, 변동률 괄호 제거 |
| **FR-20** | VIX 컨텍스트 라벨 (운영 v3) | VIX 종목 라인 끝에 `(안정)`/`(경계)`/`(공포)` inline. 임계: <20 안정 / 20–25 경계 / ≥25 공포 (CBOE 일반 가이드) |
